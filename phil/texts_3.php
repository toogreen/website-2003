...
if ($edit==$editvar) {
?>		
		<span class='blog_titre'>Add a new link entry | <a href="texts_3.php?scat=3">Exit EDIT mode</a></span>
		<form action="insert_text.php" method="post">
		<input type="hidden" name="scat" value="<? echo($scat);?>">
		Title:<input type="text" name="title" size='50' value="<? echo($title);?>"><br>
		Link:<input type="text" name="link" size='50' value="<? echo($link);?>"><br>
		Sub-section:<select name="s_section" size="1">
	  		<option value="1" selected>Text</option>
	  		<option value="2">Press Releases</option>
			<option value="3">Quoted</option>
			<option value="4">Published Photos</option>
			</select>
		<input type="Submit">
		</form>
		<br><br><br>
<?
	...
	while ($ss < 5) {
		...
		if ($ss==1) { $ssname="Text"; }
		if ($ss==2) { $ssname="Press Releases"; }
		if ($ss==3) { $ssname="Quoted"; }
		if ($ss==4) { $ssname="Published photos"; }
		...
		$num2=mysql_numrows($result2);
			...
			echo "$title ";
			echo "<br><A href='$link' target='_blank'>$link</A><br><br>";
...
<!-- begin Nedstat Pro 4.0 code --><img src="http://fr.nedstatpro.net/cgi-bin/nedstatpro.gif?name=photojou_6" width=1 height=1 align=right><!-- end Nedstat Pro 4.0 code -->
</body>
</html>
